...
function isMagickSuccess(result) {
    ...
    if (result.status !== 0) {   //status could be any value non-negative  changed@2024年3月3日20点23分
...
function runResize(imgOutput, videoHeight, options, callback) {
    ...
    mp.command_native_async({
        ...
        mp.msg.info('-here-')
        mp.msg.info(_)
        for (var tempT in result) {
            mp.msg.info(tempT);
            mp.msg.info(result[tempT]);
        }
        mp.msg.info(resizeCmds)
        mp.msg.info(__)
...
//montage -background '#336699' -geometry +4+4 mpv-shot0001.jpg mpv-shot0002.jpg montage.jpg 
